...
- The API (`api`) is an Express app, source in `api/_src/`. Locally this is typechecked and run straight from TypeScript (`api/tsconfig.json`, esbuild-bundled dev server at `dev-server.mjs`).
- **Vercel never sees that TypeScript.** Across several rounds of debugging, whatever Vercel's own build pipeline runs against `.ts` files under `api/` kept independently compiling them with settings that matched neither `api/tsconfig.json` nor `tsconfig.base.json` — producing errors that never reproduced locally, on files anywhere in `api/` regardless of nesting or underscore-prefixing, and even on a `.ts` file relocated outside `api/` entirely. The fix: `api/index.mjs` (committed, plain JS, no TypeScript) is the only file under `api/` that exists both before and after the build — it's the actual Vercel entry, and it re-exports from `./_runtime/index.mjs`. `vercel.json`'s `buildCommand` runs `pnpm run build` (typecheck against the real sources, exactly like local dev) and then `node api/vercel-postbuild.mjs`, which esbuild-bundles `api/_src/app.ts` into plain JS at `api/_runtime/index.mjs` and deletes `api/_src/` from that build, so nothing TypeScript is left under `api/` for Vercel's function builder to misinterpret. Earlier attempts generated `api/index.mjs` itself at build time (deleting/replacing whatever Vercel had detected as the entry from the git checkout) — that broke Vercel's own two-phase build (it detects functions from the pre-build checkout, then builds/traces the detected path only after `buildCommand` finishes) with "File not found: api/index.ts". Keeping the entry's path and filename permanent avoids that.
...
- `api/index.mjs` — the actual, committed Vercel serverless entry point (plain JS, re-exports from the build-time-generated `api/_runtime/index.mjs`)
- `api/vercel-postbuild.mjs` — bundles `api/_src/app.ts` into plain JS and removes the TypeScript sources, in Vercel's build sandbox only (see Deployment section)
